...
const Navigation = ({ cartItems, onRemoveFromCart, onUpdateQuantity, onCheckout }: NavigationProps) => {
  ...
  useEffect(() => {
    ...
    // initialize immediately for correct logo on first paint
    handleScroll();
  ...
  // Logo swap state and fade transition
  const [logoSrc, setLogoSrc] = useState<string>(() => (typeof window !== 'undefined' && window.scrollY >= 60 ? '/images/logo-zethnika-script.png' : '/images/logo-white.png'));
  const [isFading, setIsFading] = useState(false);
  const fadeTimer = useRef<number | null>(null);

  useEffect(() => {
    const desired = isScrolled ? '/images/logo-zethnika-script.png' : '/images/logo-white.png';
    if (desired === logoSrc) return;

    // start fade-out, swap while invisible, then fade-in
    setIsFading(true);
    if (fadeTimer.current) window.clearTimeout(fadeTimer.current);
    // 250ms total gives a luxury ~0.25-0.3s feel
  ...
  const scrollToSection = (href: string) => {
    ...
    // For anchor links like #hero, #about, etc.
  ...
  return (
    <>
      ...
      >
        {/* even taller navbar for a grander logo */}
        <div className="flex items-center justify-between h-[120px] px-6 md:px-12 lg:px-[170px]">
          <a
            href="#hero"
            onClick={(e) => {
              e.preventDefault();
          ...
          >
            <img
              ...
              className="h-44 w-auto object-contain max-w-[220px] hover:scale-105 transition-transform duration-300 drop-shadow-lg"
              style={{
      ...
      >
        ...
        <div className="relative h-full flex">
          ...
          <button
            ...
            className="absolute top-8 right-8 lg:right-12 z-10 p-2 hover:opacity-60 transition-opacity"
          >
                  ...
                  {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-4 pb-6 border-b border-gray-100">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-[#aea4a4] mt-1">{item.price.toFixed(2)} MAD</p>
            ...
            {cartItems.length > 0 && (
              <div className="p-6 border-t bg-[#fafafa]">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="font-serif text-xl">{totalPrice.toFixed(2)} MAD</span>
                </div>
                <button 
                  onClick={() => {
                    onCheckout?.();
